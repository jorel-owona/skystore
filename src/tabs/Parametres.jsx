...
import { RefreshCw, Download, CheckCircle, AlertTriangle, Globe, Printer, Sparkles, Moon, Sun, Store, ExternalLink } from 'lucide-react';
...
export default function Parametres({ addToast, isDarkMode, setIsDarkMode, globalShopName, setGlobalShopName, currentUser }) {
  ...
  const handleCheckUpdate = async () => {
    ...
    if (window.api?.checkForUpdates) {
      try {
        ...
        if (!res.success) {
          setUpdaterState({ status: 'error', error: res.error || 'Vérification GitHub indisponible' });
          if (addToast) addToast('Vérification effectuée. Téléchargement direct disponible ci-dessous.', 'info');
        }
      } catch (err) {
  ...
  const handleInstallUpdate = () => {
    if (window.api?.quitAndInstall) {
      ...
      window.open('https://github.com/jorel-owona/skystore/releases', '_blank');
    }
  };
  ...
  const handleLangChange = (langCode) => {
    ...
    window.dispatchEvent(new Event('skystore_lang_changed'));
    if (addToast) addToast(`Langue activée : ${languages.find(l => l.code === langCode)?.name}`, 'success');
  };
  ...
  const t = (key) => getTranslation(key, selectedLang);
  ...
  return (
    <div className="p-8 space-y-8 overflow-y-auto h-full max-w-7xl mx-auto custom-scrollbar pb-24">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">
              Gestion des mises à jour, langues et périphériques matériel.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* --- CARD 1 : MISES À JOUR AUTOMATIQUES (Auto-Updater + Secours GitHub) --- */}
          ...
          <div className="p-4 bg-slate-500/5 dark:bg-slate-800/30 rounded-2xl border border-white/40 dark:border-slate-700/50 space-y-3">
            ...
            {updaterState.status === 'checking' && (
              <div className="flex items-center space-x-3 text-cyan-600 dark:text-cyan-400">
                ...
                <span className="text-xs font-bold">Vérification des mises à jour sur GitHub en cours...</span>
              </div>
            )}
            ...
            {updaterState.status === 'downloaded' && (
              <div className="flex items-center justify-between">
                ...
                >
                  <Download size={14} />
                  <span>Installer & Redémarrer maintenant</span>
                </button>
              </div>
            )}

            {updaterState.status === 'error' && (
              <div className="flex flex-col space-y-2 text-rose-600 dark:text-rose-400">
                <div className="flex items-center space-x-2">
                  <AlertTriangle size={18} />
                  <span className="text-xs font-semibold">{updaterState.error || 'Impossible d\'effectuer la mise à jour automatique.'}</span>
                </div>
              </div>
            )}
          </div>

          {/* Bouton de secours / téléchargement direct sur GitHub */}
          <div className="pt-2 border-t border-slate-200/50 dark:border-slate-800/50">
            <button
              onClick={handleOpenGitHubReleases}
              className="w-full py-2.5 px-4 bg-slate-800 hover:bg-slate-900 text-white rounded-xl text-xs font-bold transition shadow-md flex items-center justify-center space-x-2"
            >
              <ExternalLink size={14} className="text-cyan-400" />
              <span>{t('directDownload')}</span>
            </button>
            <p className="text-[10px] text-slate-500 text-center mt-1">
              Au cas où le redémarrage automatique ne se déclenche pas sur votre PC.
            </p>
          </div>
        </div>
        ...
        <div className="liquid-card liquid-glass-3d p-6 space-y-5 border border-indigo-500/20">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-base font-bold text-slate-800 dark:text-slate-100">{t('langTitle')}</h2>
                <p className="text-xs text-slate-500 dark:text-slate-400">Sélectionnez la langue de l'application</p>
              </div>
            </div>
          </div>
        ...
        {/* --- CARD 3 : MATÉRIEL & IMPRESSION THERMIQUE --- */}
        ...
        {/* --- CARD 4 : PERSONNALISATION BOUTIQUE --- */}
        <div className="liquid-card liquid-glass-3d p-6 space-y-5 border border-pink-500/20">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-base font-bold text-slate-800 dark:text-slate-100">{t('shopSettings')}</h2>
